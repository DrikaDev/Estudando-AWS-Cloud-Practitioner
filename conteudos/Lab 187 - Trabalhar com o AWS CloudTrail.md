...
## Tarefa 3: Analisar os Logs do CloudTrail Usando o comando `grep`  
...
5. Cole o conteúdo para se conectar a instância via SSH e coloque o nome do arquivo labsuser.pem
6. Instância conectada! 
...
> O nome **grep** é um comando que filtra linhas com base em uma palavra ou padrão que você quer encontrar.  
> Significa:  
> - **g** — *global* (procura em todo o arquivo)  
> - **re** — *regular expression* (uma expressão ou padrão de texto)  
> - **p** — *print* (imprimir as linhas encontradas)  
> Ou seja: **“procure globalmente por este padrão e mostre (‘print’) as linhas que o contêm.”**  
> Com o tempo, essa expressão virou o comando poderoso que usamos hoje no Linux para busca de padrões em textos e arquivos.  
...
### Tarefa 3.4: Analisar os Logs Usando o comando `grep`  
Nesta etapa vamos usar o comando `grep` do Linux para filtrar e entender os eventos registrados pelo **AWS CloudTrail**.  
